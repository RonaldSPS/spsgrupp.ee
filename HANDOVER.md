...
- Supabase (`spsgrupp-live`) can auto-pause → EN/RU reviews/jobs blank + DB-user admin auth fails on Vercel. Prevention: Vercel Cron pings `app/api/cron/keepalive` daily (needs `CRON_SECRET` env in Vercel; path keeps trailing slash to skip the 308). Manual wake-up: `npm run db:restore` (local) or the "Käivita andmebaas" button in Seaded → Andmebaas (live, `app/api/spsadmn/db` — needs `SUPABASE_ACCESS_TOKEN` + `SUPABASE_PROJECT_REF` in Vercel env), then re-run `npx tsx scripts/sync-translation-fallbacks.ts`; readers fall back to `data/admin-*-translations.json`. Only a paid Supabase plan disables pausing entirely.
