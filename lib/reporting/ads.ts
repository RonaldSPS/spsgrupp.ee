/**
 * Google Ads API (GAQL) pulls for the weekly report — direct account access,
 * structured-data sibling of scripts/ads-report.ts. Campaigns incl. impression
 ...
 *
 * Auth: same service account as GA4/GSC (adwords scope), developer token from
 * the manager account. Read-only: only googleAds:search is called.
 */
...
const isBrandTerm = (t: string) => /\bsps\b|sps[\s-]?(grupp|group)/i.test(t)
