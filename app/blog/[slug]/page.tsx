    ...
    <>
      ...
      <main className="pt-[110px] pb-[80px]">
        <div className="max-w-[1200px] mx-auto px-[25px]">
          <nav className="mb-6 text-[14px] text-[#5a6474]">
          ...
          <div className="grid md:grid-cols-[65fr_35fr] gap-10">
            <article>
              <div className="flex items-center gap-3 text-[14px] text-[#5a6474] mb-4">
                <span className="text-[13px] font-semibold text-[#3abeff] bg-[#eef7fc] px-3 py-1 rounded-full">
                  {post.category}
                </span>
              ...
              <div className="text-[15px] text-[#2f353f] leading-[1.8] space-y-4">
                {post.content
                  ? post.content.split("\n").filter(Boolean).map((p, i) => {
                      const trimmed = p.trim()
                      const isSubheading = trimmed.length < 80 && trimmed.length > 5 && !/[.!?:;]$/.test(trimmed) && /^[A-ZÕÜÖÄŠŽ].*[a-zõüöäšž]/.test(trimmed)
                      return isSubheading
                        ? <h2 key={i} className="text-[22px] font-bold text-[#17345a] mt-6 mb-2">{trimmed}</h2>
                        : <p key={i}>{trimmed}</p>
                    })
                  : <p>{post.excerpt}</p>
                }
            ...
            <aside className="space-y-6">
              <div className="bg-[#eef7fc] rounded-[16px] p-6">
                <h3 className="font-bold text-[#17345a] mb-2">Autor</h3>
                <p className="text-[15px] text-[#2f353f]">{post.author}</p>
              </div>

              <div className="bg-white rounded-[16px] border border-[rgba(23,52,90,0.08)] p-6">
                <h3 className="font-bold text-[#17345a] mb-2">Kategooria</h3>
                <span className="text-[14px] font-semibold text-[#3abeff] bg-[#eef7fc] px-3 py-1.5 rounded-full">
                  {post.category}
                </span>
              </div>
              ...
              {related.length > 0 && (
                <div className="bg-white rounded-[16px] border border-[rgba(23,52,90,0.08)] p-6">
                  <h3 className="font-bold text-[#17345a] mb-3">Seotud</h3>
                    ...
                    {related.map((rp) => (
                      <Link key={rp.id} href={"/blog/" + rp.slug} className="group flex gap-3 no-underline">
                        <div className="relative w-[70px] h-[70px] rounded-[10px] overflow-hidden bg-[#eef7fc] shrink-0">
                          <Image src={rp.image} alt={rp.title} fill className="object-cover" sizes="70px" />
                        </div>
                        <div className="min-w-0">
                          <h4 className="text-[14px] font-semibold text-[#17345a] leading-tight group-hover:text-[#3abeff] line-clamp-2">
                            {rp.title}
                          </h4>
                          <div className="text-[12px] text-[#5a6474] mt-1">{formatDate(rp.date)}</div>
                        </div>
                      </Link>
                    ))}
                  </div>
                ...
                {related.map((rp) => (
                  <Link key={rp.id} href={"/blog/" + rp.slug} className="group block rounded-[16px] overflow-hidden border border-[rgba(23,52,90,0.08)] bg-white shadow-sm hover:shadow-md transition-all">
                    <div className="relative h-[180px] bg-[#eef7fc]">
                      <Image src={rp.image} alt={rp.title} fill className="object-cover group-hover:scale-[1.02] transition-transform duration-500" sizes="33vw" />
                    </div>
                    <div className="p-5">
                      <h3 className="text-[16px] font-bold text-[#17345a] leading-tight group-hover:text-[#3abeff] line-clamp-2">{rp.title}</h3>
                      <div className="text-[13px] text-[#5a6474] mt-2">{formatDate(rp.date)}</div>
                    </div>
                  </Link>
                ))}
              </div>
